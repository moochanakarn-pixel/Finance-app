...
if ($categoryId > 0) {
    $stmt = mysqli_prepare($conn, "SELECT id, name, type, sort_order FROM categories WHERE id = ? AND user_id = ? LIMIT 1");
...
if ($entryId > 0) {
    $stmt = mysqli_prepare($conn, "SELECT e.id, e.entry_date, e.amount, e.note, e.category_id FROM entries e INNER JOIN categories c ON e.category_id = c.id WHERE e.id = ? AND e.user_id = ? AND c.user_id = ? LIMIT 1");
    mysqli_stmt_bind_param($stmt, 'iii', $entryId, $userId, $userId);
    mysqli_stmt_execute($stmt);
    $rs = mysqli_stmt_get_result($stmt);
    if (!$rs || mysqli_num_rows($rs) === 0) {
        die('ไม่พบรายการ');
    ...
    $cats  = [];
    $stmt2 = mysqli_prepare($conn, "SELECT id, name, type FROM categories WHERE is_active = 1 AND user_id = ? ORDER BY FIELD(type,'income','saving','expense'), sort_order ASC, id ASC");
    mysqli_stmt_bind_param($stmt2, 'i', $userId);
    mysqli_stmt_execute($stmt2);
    $rsCats = mysqli_stmt_get_result($stmt2);
